...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  #popup .shots { display: flex; gap: .5rem; }
  #popup img { height: 220px; width: auto; image-rendering: pixelated;
               background: #000; border-radius: 3px; display: block; }
  #popup .cap { margin-top: .4rem; font-size: .8rem; color: #888; max-width: 40rem; }
  @media (max-width: 640px) {
    #popup img { height: 132px; }
    #popup .shots { flex-wrap: wrap; }
  }
</style>
</head>
<body>
...
function titleCell(d) {
  ...
  return '<span class="shot" data-img="' + escA(d.img) + '" data-slots="' +
    d.img_slots.join(',') + '" data-cap="' + escA(cap) + '">' + esc(d.title) + '</span>';
}
...
function showPop(el) {
  ...
  pop.innerHTML = '<div class="shots">' +
    el.dataset.slots.split(',').map(s =>
      '<img loading="lazy" src="../images/' + s + '/' + img + '.png" onerror="this.remove()">').join('') +
